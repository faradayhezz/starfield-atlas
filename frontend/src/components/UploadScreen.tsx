...
import { firstSupportedFileFromTransfer, INCOMING_FILE_READ_ERROR, INCOMING_FILE_UNSUPPORTED_ERROR, transferCarriesFiles } from '../incomingFile'
...
function UploadDropZone({ progress, error, filename, onBrowse, onFile, onRetry, onCancel, onIncomingError }: UploadScreenProps) {
  const [dragging, setDragging] = useState(false)
  const selectable = !progress && !error
  const onDrop = async (event: DragEvent<HTMLDivElement>) => {
    event.preventDefault(); event.stopPropagation(); setDragging(false)
    if (progress) return
    try { const file = await firstSupportedFileFromTransfer(event.dataTransfer); if (file) onFile(file); else onIncomingError(INCOMING_FILE_UNSUPPORTED_ERROR) } catch (caught) { onIncomingError(caught instanceof Error ? caught.message : INCOMING_FILE_READ_ERROR) }
  }
  return <div className={`upload-drop-zone ${dragging ? 'is-dragging' : ''}`} onDragEnter={(event) => { event.preventDefault(); if (transferCarriesFiles(event.dataTransfer)) setDragging(true) }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => { if (selectable) onBrowse() }} onKeyDown={(event) => { if (selectable && event.target === event.currentTarget && ['Enter', ' '].includes(event.key)) { event.preventDefault(); onBrowse() } }} role={selectable ? 'button' : 'group'} tabIndex={selectable ? 0 : undefined} aria-label="打开星空照片或拖放到此处">
